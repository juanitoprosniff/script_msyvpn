...
start_proxy() {
    ...
    # Si ya hay una sesión screen para este puerto, no duplicar
    if screen -list 2>/dev/null | grep -q "proxy-$port"; then
        echo "✗ Proxy puerto $port ya está corriendo"
        return 1
    fi

    # Liberar el puerto si algo lo está ocupando
    local pid_ocupado
    pid_ocupado=$(lsof -ti:$port 2>/dev/null)
    if [ -n "$pid_ocupado" ]; then
        kill -9 $pid_ocupado 2>/dev/null
        sleep 1
    fi
    ...
    if screen -list 2>/dev/null | grep -q "proxy-$port"; then
        echo "✓ Proxy :$port → SSH:$ssh_port iniciado"
        if [ "$save" = "1" ]; then
            sed -i "/^${port}|/d" /etc/proxy-python/proxies.conf 2>/dev/null
            echo "${port}|${response}|${banner}|${ssh_port}" >> /etc/proxy-python/proxies.conf
        fi
    else
        # Diagnóstico: verificar por qué falló
        echo "✗ Error al iniciar proxy en puerto $port"
        if ! command -v python3 &>/dev/null; then
            echo "  → python3 no está instalado"
        elif [ ! -f /etc/proxy-python/proxy.py ]; then
            echo "  → /etc/proxy-python/proxy.py no existe"
        elif lsof -ti:$port &>/dev/null; then
            echo "  → Puerto $port sigue ocupado por otro proceso"
        else
            echo "  → Revisa: screen -r proxy-$port"
        fi
        return 1
    fi
}
...
After=network-online.target dropbear-legacy.service
...
ExecStartPre=/bin/bash -c 'for p in 80 8080 8880 8888; do pid=$(lsof -ti:$p 2>/dev/null); [ -n "$pid" ] && kill -9 $pid 2>/dev/null || true; done'
...
# Liberar puertos 80, 8080, 8880, 8888 antes de iniciar proxies
# (Apache/Nginx/otros pueden estar ocupándolos en VPS frescos)
for _port in 80 8080 8880 8888; do
    _pid=$(lsof -ti:$_port 2>/dev/null)
    [ -n "$_pid" ] && kill -9 $_pid 2>/dev/null
done
# Detener servicios web comunes que podrían ocupar el puerto 80
systemctl stop apache2 2>/dev/null || true
systemctl stop nginx 2>/dev/null || true
systemctl stop lighttpd 2>/dev/null || true
# Deshabilitarlos para que no vuelvan a iniciar
