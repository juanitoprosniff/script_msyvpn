...
# Configuración SSL/TLS mejorada para soportar payloads y proxies
sslVersion = all
options = NO_SSLv2
options = NO_SSLv3
...
# Opciones de compatibilidad para payloads HTTP/CONNECT/WebSocket
TIMEOUTbusy = 300
TIMEOUTclose = 10
TIMEOUTidle = 300
debug = 4
...
# Permitir protocolo transparente (payload pass-through)
protocol = connect
TIMEOUTbusy = 300
...
SSH_PORT = 143
RESPONSE_CODE = '101'
BANNER_TEXT = '\033[1;31mMSY VPN SCRIPT\033[0m'
...
USEREOF
            ...
            4)
                ...
                case $proxy_opt in
                    1)
                        ...
                        read -p "Banner text [MSY VPN SCRIPT]: " banner
                        banner=${banner:-MSY VPN SCRIPT}
                        echo ""
            ...
            6)
                ...
                echo ""
                read -p "Nuevo texto [MSY VPN SCRIPT]: " new_banner
                new_banner=${new_banner:-MSY VPN SCRIPT}
...
restore_proxies() {
    ...
    if [ ! -f "$conf" ] || [ ! -s "$conf" ]; then
        ...
        echo "80|101|MSY VPN SCRIPT|143"   > "$conf"
        echo "8080|101|MSY VPN SCRIPT|143" >> "$conf"
        echo "8880|101|MSY VPN SCRIPT|143" >> "$conf"
    fi
...
[ ! -f /etc/proxy-python/proxies.conf ] && {
    echo "80|101|MSY VPN SCRIPT|143"   > /etc/proxy-python/proxies.conf
    echo "8080|101|MSY VPN SCRIPT|143" >> /etc/proxy-python/proxies.conf
    echo "8880|101|MSY VPN SCRIPT|143" >> /etc/proxy-python/proxies.conf
}
