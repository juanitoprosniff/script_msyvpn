...
# Crear certificado sin prompt interactivo
openssl req -new -key /root/stunnel.key -x509 -days 3650 -out /root/stunnel.crt \
    -subj "/C=US/ST=State/L=City/O=MSY-VPN/CN=t.me/FREEINTERNETVPNMSY" > /dev/null 2>&1

# Combinar key y cert en un solo archivo
cat /root/stunnel.crt /root/stunnel.key > /root/stunnel.pem

# Mover a directorio de stunnel
mv /root/stunnel.pem /etc/stunnel/

# Limpiar archivos temporales
rm -rf /root/stunnel.crt /root/stunnel.key

chmod 600 /etc/stunnel/stunnel.pem

# Configuración SIMPLE de Stunnel (compatible con WebSocket, Payload, Remote Proxy)
