...
echo "Modificando identificador SSH a 'SSH-2.0-ByJunitoProSniff'..."
sed -i 's/#define LOCAL_IDENT "SSH-2.0-dropbear_" DROPBEAR_VERSION/#define LOCAL_IDENT "SSH-2.0-ByJunitoProSniff"/' default_options.h 2>/dev/null || true
sed -i 's/SSH-2.0-dropbear/SSH-2.0-ByJunitoProSniff/g' default_options.h 2>/dev/null || true
...
menu_principal() {
    while true; do
        ...
        echo -e "${CYAN}╔══════════════════════════════════════╗${NC}"
        echo -e "${CYAN}║${NC}  \033[40m\033[1;33m   MSY VPN SCRIPT   \033[0m              ${CYAN}║${NC}"
        echo -e "${CYAN}╚══════════════════════════════════════╝${NC}"
...
✓ Dropbear 2016:     puerto 143 (SSH-2.0-ByJunitoProSniff)
...
MEJORAS v10 CUSTOM:
✓ Dropbear 2016.74 con identificador SSH-2.0-ByJunitoProSniff
