...
# ====================
# INSTALAR WSTUNNEL (SSL OVER WEBSOCKET)
# ====================
echo "Instalando wstunnel (SSL over WebSocket)..."
...
case $ARCH in
    ...
    armv7l)
        ...
        ;;
    *)
        echo "⚠ Arquitectura $ARCH no soportada para wstunnel, saltando..."
        WSTUNNEL_ARCH=""
        ;;
esac
...
if [ -n "$WSTUNNEL_ARCH" ]; then
    ...
    WSTUNNEL_VERSION=$(curl -s https://api.github.com/repos/erebe/wstunnel/releases/latest | grep '"tag_name"' | cut -d'"' -f4)
    
    if [ -n "$WSTUNNEL_VERSION" ]; then
        # Descargar binario
        WSTUNNEL_FILE="wstunnel_${WSTUNNEL_VERSION#v}_linux_${WSTUNNEL_ARCH}.tar.gz"
        wget -q "https://github.com/erebe/wstunnel/releases/download/${WSTUNNEL_VERSION}/${WSTUNNEL_FILE}" -O /tmp/wstunnel.tar.gz
        
        if [ -f /tmp/wstunnel.tar.gz ]; then
            tar -xzf /tmp/wstunnel.tar.gz -C /tmp/
            mv /tmp/wstunnel /usr/local/bin/wstunnel
            chmod +x /usr/local/bin/wstunnel
            rm -f /tmp/wstunnel.tar.gz
            
            # Crear servicio para wstunnel SSL (puerto 8443)
            cat > /etc/systemd/system/wstunnel-ssl.service <<'WSEOF'
[Unit]
Description=WSTunnel SSL over WebSocket - Puerto 8443
After=network.target

[Service]
Type=simple
ExecStart=/usr/local/bin/wstunnel server wss://[::]:8443 --restrict-to 127.0.0.1:143
Restart=always
RestartSec=3
KillMode=process
...
WSEOF
            ...
            # Activar e iniciar servicio
            systemctl daemon-reload
            systemctl enable wstunnel-ssl
            systemctl start wstunnel-ssl
            
            echo "✓ WSTunnel instalado (puerto 8443 → SSH 143)"
            echo "  Uso: wss://TU_IP:8443 (compatible con payload/remote proxy)"
        else
            echo "⚠ Error descargando wstunnel"
        fi
    else
        echo "⚠ No se pudo obtener versión de wstunnel"
    fi
fi
...
${GREEN}✓${NC} WSTunnel SSL:      8443→143 (WebSocket + SSL)
...
${GREEN}✓${NC} WSTunnel: SSL over WebSocket (compatible con payload/proxy remoto)
${GREEN}✓${NC} Puerto 443 Stunnel → Dropbear 2016 (puerto 143)
${GREEN}✓${NC} Banners SSH con HTML styling (apps cliente)
...
- WSTunnel SSL: 8443→143 (WebSocket + SSL)
...
WSTUNNEL:
- Puerto 8443 → Dropbear 2016 (143) 
- Compatible con: payload, proxy remoto, CDN
