#!/bin/bash
# Script SSH VPN Completo - V9
# - OpenSSH: 22, Dropbear: 90, Dropbear Legacy (Ubuntu 18): 143
...
# ====================
# INSTALAR DROPBEAR LEGACY (UBUNTU 18 / 2016) EN PUERTO 143
# Compatible con clientes SSH más antiguos
# ====================
echo "Configurando Dropbear Legacy (Ubuntu 18 / 2016) en puerto 143..."

# Verificar si hay otra instancia de dropbear disponible o usar el mismo binario
# con configuración separada en otro puerto
cat > /etc/systemd/system/dropbear-legacy.service <<'DBEOF'
[Unit]
Description=Dropbear SSH Legacy (Ubuntu 18 compatible) - Puerto 143
After=network.target
Wants=network.target

[Service]
Type=simple
EnvironmentFile=/etc/default/dropbear-legacy
ExecStart=/usr/sbin/dropbear -F -p 143 -b /etc/dropbear/banner-legacy.txt -K 60 -I 300 -P /var/run/dropbear-legacy.pid
...
cat > /etc/default/dropbear-legacy <<'DBLEOF'
# Dropbear Legacy - Ubuntu 18 / 2016 compatible
DROPBEAR_LEGACY_PORT=143
DBLEOF

cat > /etc/dropbear/banner-legacy.txt <<'BANEOF'
═══════════════════════════════════════
          MSY VPN
   Dropbear Legacy (Ubuntu 18)
   t.me/FREEINTERNETVPNMSY
═══════════════════════════════════════
BANEOF

# Verificar que el binario dropbear soporta la bandera -F (foreground)
if /usr/sbin/dropbear -F --help 2>&1 | grep -q "usage\|dropbear"; then
    systemctl daemon-reload
    systemctl enable dropbear-legacy
    systemctl start dropbear-legacy
    echo "✓ Dropbear Legacy iniciado en puerto 143"
else
    echo "⚠ Dropbear no soporta modo foreground, usando método alternativo..."
    # Método alternativo: usar dropbear directamente
    /usr/sbin/dropbear -p 143 -b /etc/dropbear/banner-legacy.txt -K 60 -I 300 2>/dev/null &&         echo "✓ Dropbear Legacy iniciado en puerto 143 (modo daemon)" ||         echo "⚠ No se pudo iniciar Dropbear Legacy en 143"
fi
            ...
            4)
                ...
                case $proxy_opt in
                    1)
                        ...
                        echo "  1) 22  - OpenSSH"
                        echo "  2) 90  - Dropbear moderno (recomendado)"
                        echo "  3) 143 - Dropbear Ubuntu 18 (2016)"
...
stop_proxy() {
    ...
    # Eliminar del archivo permanente
    sed -i "/^${port}|/d" /etc/proxy-python/proxies.conf 2>/dev/null
...
restore_proxies() {
    ...
    if [ ! -f "$conf" ] || [ ! -s "$conf" ]; then
        ...
        echo "80|101|MSY VPN|90"   > "$conf"
        echo "8080|101|MSY VPN|90" >> "$conf"
        echo "8880|101|MSY VPN|90" >> "$conf"
    fi
...
[ ! -f /etc/proxy-python/proxies.conf ] && {
    echo "80|101|MSY VPN|90"   > /etc/proxy-python/proxies.conf
    echo "8080|101|MSY VPN|90" >> /etc/proxy-python/proxies.conf
    echo "8880|101|MSY VPN|90" >> /etc/proxy-python/proxies.conf
}
