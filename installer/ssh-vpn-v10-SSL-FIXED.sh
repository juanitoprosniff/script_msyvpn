...
# ====================
# STUNNEL (PUERTO 443) - MEJORADO Y FUNCIONAL
# ====================
echo "Configurando Stunnel en puerto 443..."

# Generar certificado SSL con configuración mejorada
openssl genrsa -out /etc/stunnel/stunnel.key 2048 > /dev/null 2>&1

openssl req -new -key /etc/stunnel/stunnel.key -x509 -days 3650 -out /etc/stunnel/stunnel.crt \
    -subj "/C=US/ST=California/L=LA/O=MSY-VPN/CN=msyvpn.com" > /dev/null 2>&1
...
# Configuración de Stunnel MEJORADA (funciona con todos los métodos)
...
; Cifrados compatibles (incluye los que usan los proxies)
ciphers = ECDHE-RSA-AES128-GCM-SHA256:ECDHE-RSA-AES256-GCM-SHA384:ECDHE-RSA-AES128-SHA256:ECDHE-RSA-AES256-SHA384:AES128-GCM-SHA256:AES256-GCM-SHA384:AES128-SHA256:AES256-SHA256:AES128-SHA:AES256-SHA:DES-CBC3-SHA:HIGH:!aNULL:!eNULL:!EXPORT:!DES:!MD5:!PSK:!RC4
...
; Servicio SSL en puerto 444 -> OpenSSH 22
[openssh-ssl]
client = no
accept = 0.0.0.0:444
...
; Servicio SSL en puerto 777 -> Dropbear 143
[dropbear-ssl-777]
client = no
accept = 0.0.0.0:777
connect = 127.0.0.1:143
...
if systemctl is-active --quiet stunnel4; then
    echo "✓ Stunnel SSL configurado correctamente en puertos 443, 444, 777"
else
...
${GREEN}✓${NC} Stunnel SSL:       puerto 443 → 143
...
${CYAN}MEJORAS v10 OPTIMIZADO:${NC}
${GREEN}✓${NC} Dropbear 2016.74 con identificador SSH-2.0-ByJuanitoProSniff
${GREEN}✓${NC} Stunnel4 configuración SIMPLE (soporta TODOS los métodos)
${GREEN}✓${NC} Compatible: WebSocket, Payload, Remote Proxy, Direct, etc.
${GREEN}✓${NC} Banners HTTP con HTML styling (fondo negro, texto amarillo)
${GREEN}✓${NC} Menú colorido \"MSY VPN SCRIPT\"
${GREEN}✓${NC} Sistema simplificado y optimizado
${GREEN}✓${NC} Solo servicios esenciales (sin Squid, sin Dropbear 90)
