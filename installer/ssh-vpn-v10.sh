...
count_users() {
    # Obtener PIDs de procesos sshd y dropbear con conexiones establecidas
    # en todos los puertos SSH: 22, 90, 143 y puertos Stunnel: 443, 444, 777
    local total=0

    # Contar sesiones SSH activas por usuario (método más confiable)
    # ss -tp muestra "users:(("sshd",pid=...,fd=...))"
    local ssh_sessions=$(ss -tnp state established 2>/dev/null \
        | grep -E ':(22|90|143|443|444|777)\s' \
        | grep -v '127\.0\.0\.1' \
        | grep -oP 'pid=\K[0-9]+' \
        | while read pid; do
            # Obtener el usuario del proceso
            cat /proc/$pid/status 2>/dev/null | grep "^Uid:" | awk '{print $2}'
          done \
        | sort -u \
        | wc -l)

    # Complementar con who/w para sesiones interactivas
    local who_sessions=$(who 2>/dev/null | awk '{print $1}' | sort -u | wc -l)

    # Usar el mayor de los dos conteos
    if [ "$ssh_sessions" -gt "$who_sessions" ] 2>/dev/null; then
        total=$ssh_sessions
    else
        total=$who_sessions
    fi
...
# Obtener nombre de usuario a partir de un PID
_get_user_from_pid() {
    local pid=$1
    local uid=$(cat /proc/$pid/status 2>/dev/null | grep "^Uid:" | awk '{print $2}')
    if [ -n "$uid" ]; then
...
list_connected_users() {
    ...
    # Recolectar todos los usuarios con sesión activa en cualquier puerto SSH
    declare -A user_ports

    for port in 22 90 143 443 444 777; do
        local label
        case $port in
            22)  label="OpenSSH" ;;
            90)  label="Dropbear" ;;
            143) label="Dropbear Legacy" ;;
            443) label="SSL/443" ;;
            444) label="SSL/444" ;;
            777) label="SSL/777" ;;
        esac

        # Obtener PIDs de conexiones establecidas en este puerto (excluyendo loopback)
        local pids=$(ss -tnp state established 2>/dev/null \
            | grep ":${port}\s" \
            | grep -v '127\.0\.0\.1' \
            | grep -oP 'pid=\K[0-9]+')

        for pid in $pids; do
            local uname=$(_get_user_from_pid $pid)
            [ -z "$uname" ] && continue
            # Acumular puertos por usuario
            if [ -z "${user_ports[$uname]}" ]; then
                user_ports[$uname]="$label"
            else
                # Agregar puerto solo si no está ya
                echo "${user_ports[$uname]}" | grep -q "$label" || \
                    user_ports[$uname]="${user_ports[$uname]}, $label"
            fi
        done
    done

    # También incluir sesiones detectadas por 'who' (consola/tty)
    while IFS= read -r line; do
        local uname=$(echo "$line" | awk '{print $1}')
        [ -z "$uname" ] && continue
        if [ -z "${user_ports[$uname]}" ]; then
            user_ports[$uname]="Consola"
        fi
    done < <(who 2>/dev/null)
    ...
    if [ ${#user_ports[@]} -eq 0 ]; then
        echo ""
        echo "  Sin usuarios conectados"
    else
        echo ""
        local n=1
        for uname in "${!user_ports[@]}"; do
            echo "  $n) $uname  [${user_ports[$uname]}]"
            n=$((n+1))
        done
    fi

    echo ""
    echo "  Total: ${#user_ports[@]} usuario(s) conectado(s)"
    echo "═══════════════════════════════════════"
}
...
echo -e "
${CYAN}╔══════════════════════════════════════════════╗${NC}
${CYAN}║${NC}   ${GREEN}✓ INSTALACIÓN COMPLETADA - v10 SSL+${NC}    ${CYAN}║${NC}
${CYAN}╚══════════════════════════════════════════════╝${NC}
...
${GREEN}STUNNEL SSL/TLS (COMPLETO):${NC}
${GREEN}✓${NC} Puerto 443 → Dropbear 143
${GREEN}✓${NC} Puerto 444 → OpenSSH 22
${GREEN}✓${NC} Puerto 777 → Dropbear 143 (Experimental)

${CYAN}Soporta todos los métodos:${NC}
${GREEN}✓${NC} SSL Directo (IP/Dominio + 443 + SNI)
${GREEN}✓${NC} SSL Inverso (Host remoto + SNI tu dominio)
${GREEN}✓${NC} WebSocket sobre SSL/TLS
${GREEN}✓${NC} Payload personalizado
${GREEN}✓${NC} Proxy Protocol
${GREEN}✓${NC} SNI Flexible (acepta cualquier dominio)
${GREEN}✓${NC} Certificado Wildcard (*)
${GREEN}✓${NC} TLS 1.2 y 1.3

${GREEN}PROXIES:${NC}
${GREEN}✓${NC} Python HTTP:       80, 8080, 8880, 8888 → SSH 143
...
${CYAN}MEJORAS v10 SSL+ COMPLETO:${NC}
${GREEN}✓${NC} Dropbear 2016.74 con identificador SSH-2.0-ByJuanitoProSniff
${GREEN}✓${NC} Stunnel con soporte COMPLETO: SSL Directo + WebSocket + Payload
${GREEN}✓${NC} SNI Flexible: Acepta tu dominio, google.com, o cualquier host
${GREEN}✓${NC} Compatible con TODOS los métodos de conexión
${GREEN}✓${NC} Certificado SSL wildcard para máxima compatibilidad
${GREEN}✓${NC} Proxy Protocol y compression zlib
${GREEN}✓${NC} Timeouts largos para WebSocket (24 horas)
...
${YELLOW}MÉTODOS DE CONEXIÓN:${NC}
1) ${CYAN}SSL Directo:${NC} IP:443 + user/pass + SNI:www.google.com
2) ${CYAN}SSL Inverso:${NC} Host:google.com + SNI:tu-dominio.com
3) ${CYAN}WebSocket:${NC} wss://tu-dominio.com:443
4) ${CYAN}Con Payload:${NC} GET / HTTP/1.1[crlf]Host: tu-dominio[crlf]...
