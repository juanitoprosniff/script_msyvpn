...
menu_principal() {
    ...
    while true; do
        ...
        cat <<EOF
╔══════════════════════════════════════════════╗
║         MSY VPN Panel - v10 SSL+             ║
╚══════════════════════════════════════════════╝
 IP: $IP        Online: $USERS_ONLINE cliente(s)
...
count_users() {
    # Contar IPs externas unicas con conexion TCP ESTABLISHED (cualquier puerto)
    # Captura SSH, Dropbear, HTTP proxy, Stunnel, WebSocket - sin importar el puerto
    ss -tn state established 2>/dev/null \
        | awk 'NR>1 {
            split($5, a, ":");
            # Para IPv4 la IP es el penultimo campo separado por ":"
            # Reconstruir IP descartando el ultimo segmento (puerto)
            n=split($5, b, ":");
            ip="";
            for(i=1;i<n;i++) ip=(ip=="")?b[i]:ip":"b[i];
            # Para IPv4 simple el split queda en a[1]
            if(n==2) ip=a[1];
            print ip
          }' \
        | grep -v '^$' \
...
list_connected_users() {
    ...
    # Recolectar IP externa → servicio(s)
    declare -A ip_svcs

    while read -r local_addr remote_addr; do
        # Extraer IP remota (quitar puerto final)
        local remote_ip
        remote_ip=$(echo "$remote_addr" | sed 's/:[0-9]*$//' | tr -d '[]')
        # Extraer puerto local
        local local_port
        local_port=$(echo "$local_addr" | rev | cut -d: -f1 | rev)

        # Ignorar loopback y privadas
        echo "$remote_ip" | grep -qE '^(127\.|::1|10\.|192\.168\.|172\.(1[6-9]|2[0-9]|3[01])\.)' && continue
        [ -z "$remote_ip" ] && continue

        # Nombre del servicio por puerto local
        local svc
        case "$local_port" in
            22)   svc="SSH-22"        ;;
            90)   svc="Dropbear-90"   ;;
            143)  svc="Dropbear-143"  ;;
            443)  svc="SSL-443"       ;;
            444)  svc="SSL-444"       ;;
            777)  svc="SSL-777"       ;;
            80)   svc="Proxy-80"      ;;
        ...
        if [ -z "${ip_svcs[$remote_ip]}" ]; then
            ip_svcs[$remote_ip]="$svc"
        else
            echo "${ip_svcs[$remote_ip]}" | grep -q "$svc" || \
                ip_svcs[$remote_ip]="${ip_svcs[$remote_ip]} | $svc"
        fi
    done < <(ss -tn state established 2>/dev/null | awk 'NR>1 {print $4, $5}')

    if [ ${#ip_svcs[@]} -eq 0 ]; then
        echo "  Sin clientes conectados"
    else
        local n=1
        for ip in "${!ip_svcs[@]}"; do
            echo "  $n) Cliente-$n  [${ip_svcs[$ip]}]"
            n=$((n+1))
        done
    fi

    echo ""
    echo "  Total: ${#ip_svcs[@]} cliente(s) conectado(s)"
    echo "═══════════════════════════════════════════════"
}
...
get_system_stats() {
    ...
    # Uso CPU
    local cpu_usage
    cpu_usage=$(awk 'NR==1{u=$2+$4; i=$5} END{print u, i}' /proc/stat; sleep 0.3; awk 'NR==1{u=$2+$4; i=$5} END{print u, i}' /proc/stat) 2>/dev/null
    # Metodo simple con top
    cpu_usage=$(top -bn2 2>/dev/null | grep "Cpu(s)" | tail -1 | awk '{
        for(i=1;i<=NF;i++){
            if($i~/^[0-9]/ && $(i+1)~/us/){printf "%.0f", $i; exit}
            if($i~/us,/){gsub(/[^0-9.]/,"",$i); printf "%.0f", $i; exit}
        }
    }')
...
menu_principal() {
    while true; do
        ...
        echo -e "${CYAN}╚══════════════════════════════════════════════╝${NC}"
        echo -e "${GREEN}IP:${NC} ${YELLOW}$IP${NC}   ${GREEN}Online:${NC} ${YELLOW}${USERS_ONLINE} cliente(s)${NC}"
